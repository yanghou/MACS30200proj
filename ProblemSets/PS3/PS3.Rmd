...
```{r}
biden.out <- amelia(as.data.frame(data2), m = 5, idvars = c("female", "dem", "rep", "educ", "age"))
models_trans_imp <- data_frame(data = biden.out$imputations) %>%
  mutate(model = map(data, ~ lm(biden ~ age + educ + female,data = .x)),coef = map(model, tidy)) %>%unnest(coef, .id = "id")
...
lm5 = lm(biden ~ age + female + educ,data=data)
tidy(lm5) %>%
